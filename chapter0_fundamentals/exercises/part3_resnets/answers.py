...
class LitConvNetTest(pl.LightningModule):
    ...
    def _shared_train_val_step(
        ...
        X, y = batch
        y_hat = self(X)
        return y_hat, F.cross_entropy(y_hat, y)

    def training_step(self, batch: Tuple[Tensor, Tensor], batch_idx: int) -> Tensor:
        _, loss = self._shared_train_val_step(batch)
        self.log("train/loss", loss)
        return loss

    def validation_step(self, batch: Tuple[Tensor, Tensor], batch_idx: int) -> None:
        logits, loss = self._shared_train_val_step(batch)
        self.log("validation/loss", loss)
        preds = logits.argmax(dim=1)
        acc = t.sum(preds == batch[1]) / len(preds)
...
class BatchNorm2d(nn.Module):
    ...
    def forward(self, x: t.Tensor) -> t.Tensor:
        ...
        if self.training:
            ...
            self.running_mean = (1 - m) * self.running_mean + m * mean.squeeze()
            self.running_var = (1 - m) * self.running_var + m * var.squeeze()
